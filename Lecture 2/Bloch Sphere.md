...
To plot it, you have 2 important angles. $\theta$ which is the angle with the z-axis. $\theta = 0 \implies \text{spin up}$ (in case of electrons, for example. $\ket{0}$ in general cases). $\theta = \pi = 180\text{deg}$ (mesaured in radians, obviously) is 100% spin down (or $\ket{1}$).

For anywhere in between, there are loads of possible states, those make a circle around a 2D span of the qubit vector. This necessitates another angle $\phi$. This is the phase. It doesn't govern probabilites. But it allows for different qubits to interact with one another.
